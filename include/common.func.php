...
function writetofile($text, $filename) {
	...
	if(!is_dir($path['dirname'])) {
		jd_mkdir($path['dirname']);
	}
	if(!$fp = @fopen($filename, 'w')) {
		aexit('<a href="http://www.jdcms.com/manual/write-permission.htm" target="_blank">Fatal error: '.$filename.' is not writable!</a>');
		return false;
	} else {
		flock($fp, LOCK_EX);
		fwrite($fp, $text);
...
function eventlog($log, $type = '', $weight = 10) {
	...
	$logfile = JD_ROOT.'logs/'.$type.date('Ymd', $time).'.log';
	...
	jdlog($time."\t".$onlineip."\t$query\t".$log, $logfile);
}

function jd_mkdir($dirname) {
...
function jd_touch($file) {
	$dir = dirname($file);
	jd_mkdir($dir);
	return touch($file);
}

function jd_copy($source, $target) {
	$targetdir = dirname($target);
	if(!file_exists($targetdir)) jd_mkdir(dirname($target));
...
function debug($variable, $exit = 0, $type = 0) {
	...
	if($type != 3) $info = jd_htmlspecialchars($info);
...
function uploadfile($file, $newname) {
	...
	if(!is_dir($path['dirname'])) jd_mkdir($path['dirname']);
	return move_uploaded_file($file, $newname);
}

function jd_ftpput($source, $target, $server = '') {
	global $attachserver;
	static $conn_id;
...
function displaytemplate($template, $variables = array()) {
	global $lan, $_vc;
	$paths = array('customtemplate' => JD_ROOT."configs/templates", 'template' => CORE_ROOT.'templates', 'cache' => JD_ROOT."cache/templates");
	$tpl = new tpl($paths);
	$variables['vc'] = $_vc;
	...
	$variables['header_charset'] = $GLOBALS['header_charset'];
	$variables['jd_url'] = JD_URL;
	$variables['core_url'] = CORE_URL;
	if(!empty($GLOBALS['setting_sitename'])) $variables['sitename'] = $GLOBALS['setting_sitename'];
	if(file_exists(JD_ROOT.'configs/customer.css')) $variables['customcss'] = 1;
...
function jd_substr($str, $start, $len = 0xFFFFFFFF, $strip = '', $charset_force = '') {
	global $charset;
...
function jd_replace($find, $replace, $str, $caseless = 1, $count = -1) {//$caseless�Ƿ����ִ�Сд��0Ϊ������
	if(!is_array($find)) {
		$find = array($find);
	}
	if(!is_array($replace)) {
...
function jd_array_replace($finds, $replaces, $str) {
	$_str = $str;
	foreach($finds as $key => $value) {
		$r = '';
		if(isset($replaces[$key])) $r = $replaces[$key];
		$_str = jd_replace($value, $r, $_str);
	}
	return $_str;
}
...
function jd_htmlspecialchars($array) {
	global $charset;
	...
	foreach($array as $key => $value) {
		if(is_array($value)) {
			$array[$key] = jd_htmlspecialchars($value);
		} elseif(is_object($value)) {
			$array[$key] = jd_htmlspecialchars(get_object_vars($value));
...
function jd_md5($string, $short = 0, $time = 1) {
	for($i = $time; $i >= 1; $i --) {
		$string = md5($string);
	}
...
function readfromurl($url, $convertcharset = 0, $type = '') {
	...
	$agent = 'Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1; SV1; JDCMS)';
	if($type == 'wget') {
		$_tmp = JD_ROOT.'cache/'.md5($url);
...
function monitor($sign = '', $id = 0) {
	...
	$exe_times[$id][] = jdmicrotime();
	$exe_mems[$id][] = jd_memused();
	$exe_signs[$id][] = $sign;
}
...
function monitor_log($logfile = '', $id = 0) {
	...
	if($logfile == '') {
		...
		jdlog(implode("\t", $exes), JD_ROOT.'logs/'.$logfile);
	}
...
function jd_strrpos($string, $findme) {
	if(PHP_VERSION < '5.0') {
...
function jd_xor($string, $key = '') {
	if('' == $string) return '';
	if('' == $key) $key = 'jdcms';
	$len1 = strlen($string);
	$len2 = strlen($key);
	if($len1 > $len2) $key = str_repeat($key, ceil($len1 / $len2));// ceil 获取比他大的最接近的整数
	return $string ^ $key;
}
...
function replacekeyword($text, $replace, $to, $caseless = 1, $count = -1) {
	...
	if(!empty($_replace) && !empty($_to)) {
		$text = jd_replace($_replace, $_to, $text, $caseless, $count);
	}
	...
	if(!empty($_replace2) && !empty($_to2)) {
		$text = jd_replace($_replace2, $_to2, $text, $caseless, $count);
	}
	$text = jd_replace($replace, $to, $text, $caseless, $count);
	if(!empty($_replace) && !empty($_to)) {
		$text = jd_replace($_to, $_replace, $text, $caseless, $count);
	}
	if(!empty($_replace2) && !empty($_to2)) {
		$text = jd_replace($_to2, $_replace2, $text, $caseless, $count);
	}
	return $text;
}

function jd_strtotime($str) {
...
function jd_if($variable, $if, $else = '') {
...
function jd_ifnotempty($variable, $if, $else = '') {
	if($variable != '') {
		return $if;
	} else {
		return $else;
...
function jd_hmac($algo, $data, $key, $raw_output = false) {
...
function jdsetcookie($name, $value, $expire = 0) {
	global $cookiepre;
	return setcookie($cookiepre.'_'.$name, $value, $expire);
}

function jdgetcookie($name) {
	global $cookiepre;
...
function httppost($url, $params, $cookies = array(), $headers = array()) {
	if(empty($headers['agent'])) {
		$agent = 'Mozilla/5.0 (Windows; U; Windows NT 5.1; zh-CN; rv:1.9.2.13) JDCMS';
	} else {
		$agent = $headers['agent'];
	}
	$referer = '';
...
function ifinstalled() {
	return file_exists(JD_ROOT.'configs/install.lock');
}

function setinstalled() {
	jd_touch(JD_ROOT.'configs/install.lock');
}
