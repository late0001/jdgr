...
function createconfig($configs = array()) {
	...
	foreach($array_config as $config) {
		...
		if($config == 'codekey' && $value == 'jdcms') $value = false;
		if($config == 'cookiepre' && $value == 'jdcms') $value = false;
		if($value === false) {
			if($config == 'ifdebug') {
				$value = '0';
...
function render_template($template, $pagevariables = array(), $createhtml = 0, $show = 0) {
	...
	global $template_path, $tpl, $user, $thetime, $lan, $lr, $header_charset, $globalvariables, $sections, $setting_storemethod, $homepage, $setting_defaultfilename, $sysname, $sysedition;
	if(empty($pagevariables['subtemplate']) && empty($pagevariables['systemplate']) && strpos($template, '/') === false) $template = ','.$template;
	$paths = array('template' => JD_ROOT."configs/templates/$template_path", 'defaulttemplate' => JD_ROOT.'templates/fore', 'cache' => JD_ROOT."cache/foretemplates");
	if(!isset($tpl)) $tpl = new tpl($paths);
	$GLOBALS['tpl'] = $tpl;
	...
	if(isset($user)) {
		$variables['userphpname'] = 'jdcms_user.php';
		$variables['lan'] = $lan;
	...
	foreach($_GET as $key => $value) {
		$variables['get_'.$key] = jd_htmlspecialchars($value);
		$variables['get_d_'.$key] = $value;
		$variables['get_u_'.$key] = urlencode($value);
	}
	foreach($_COOKIE as $key => $value) {
		$variables['cookie_'.$key] = jd_htmlspecialchars($value);
	...
	$function = 'jdinclude,jdincludeurl,getitems,gettexts,getcategories,getcomments,getlists,monitor,getindexs,getpaging,ifhassubcategories,getattachments,getkeywords,getsqls,getinfo,getuser,getpictures,jdecho';
	$tpl->regfunction($function);
	$plugins = getcache('plugins');
	if(!empty($plugins)) {
		foreach($plugins as $plugin) {
			$pluginkey = str_replace('.template.php', '', $plugin);
			require_once(JD_ROOT.'plugins/'.$plugin);
			$tpl->regfunction($pluginkey);
		}
	}
...
function get_item_data($id, $template = '', $params = array()) {
	global $template_path, $db, $tablepre, $lan, $thetime, $system_root, $lr, $header_charset, $setting_homepage, $setting_ifhtml, $sections, $setting_storemethod, $setting_richtext, $homepage, $attachurl;
	...
	$templatehtml = readfromfile(JD_ROOT.'configs/templates/'.$template_path.'/,'.$variables['template']);
	...
	if(!empty($item['ext'])) {
		$itemextvalues = jd_unserialize($db->get_by('value', 'item_exts', "id='{$id}'"));
...
function getkeywordscache() {
	global $codekey;
	$_keywords = array();
	if($fp = @fopen(JD_ROOT.'configs/keywords.txt', 'r')) {
		while(!feof($fp)) {
			$_line = trim(fgets($fp));
...
function getsedata($sid) {
	...
	return $se + jd_unserialize($se['value']);
}
...
function renderkeywords($text, $keywords) {
	...
	if(!empty($setting_keywordslink)) {
		...
		foreach($keywords as $keyword) {
			...
			$_to = jd_replace('[keywordinurl]', urlencode($keyword), $setting_keywordslink);
			$_to = jd_replace('[keyword]', $keyword, $_to);
			$replace[] = $keyword;
			$to[] = $_to;
		}
	}
...
function changeuserpassword($uid, $password) {
	global $db;
	$db->update('users', array('password' => jd_md5($password, 1, 2)), "id='$uid'");
}
