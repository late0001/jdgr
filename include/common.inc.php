<?php 
//CORE_ROOT 在index.php中已经定义网站根目录(相对路径)
if (!define('CORE_ROOT')) exit;
...
if(isset($_SERVER['HTTP_HOST'])){
	...
	if(!isset($currenturl) && isset($_SERVER['HTTP_X_REWRITE_URL'])) 
...
if(ifinstalled()){
	if(file_exists(JD_ROOT.'configs/config.inc.php')) require JD_ROOT.'configs/config.inc.php';
} else {
	$template_path = 'jd';
	$timedifference = 0;
	$charset = 'utf8';
...
if(!isset($fore_root)) $fore_root = JD_ROOT.'../'; //根目录的上一级目录
define('FORE_ROOT', $fore_root);
if(isset($jd_url)) define('JD_URL', $jd_url);

if($__callmode == 'web') {
	$_p1 = strrpos(substr(JD_ROOT, 0, -1), $separator);
	$system_root = substr(JD_ROOT, $_p1 + 1, -1);
	$_p2 = jd_strrpos($currenturl, "/{$system_root}/") + 1;
	if($_p2 == 1 && strrpos($currenturl, "/{$system_root}/") === false) {
		...
		$_p2 = jd_strrpos($_u1, '/') + 1;
	}
	if(!isset($fore_url)) $fore_url = substr($currenturl, 0, $_p2);
	define('JD_URL', $fore_url.$system_root.'/');
	if(!isset($core_url)) {
		define('CORE_URL', JD_URL);
	} else {
...
if(!empty($setting_systemurl)) {
	...
	if('web' == $__callmode) $systemurl = JD_URL;
}
