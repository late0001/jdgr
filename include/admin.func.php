...
function ifallowtheme() {
	if(file_exists(JD_ROOT.'configs/theme.lock')) return false;
	return true;
}

function preparetheme() {
	@unlink(JD_ROOT.'configs/theme.lock');
}
function finishtheme() {
	@jd_touch(JD_ROOT.'configs/theme.lock');
}

function ifcustomed() {
	if(file_exists(JD_ROOT.'configs/custom.menu.xml')) return true;
	return false;
}
...
function get_select($type, $root = 0) {
	if($type == 'category') {
		...
		foreach($sections as $section) {
			$selectsections .= "<option value=\"$section[id]\">".jd_htmlspecialchars($section['section'])."</option>\n";
		}
...
function renderinput($name, $type = 'string', $standby = '', $value = '') {
	...
	$input = '';
	if($type == 'string') {
		$input = '<input type="text" name="'.$name.'" value="'.jd_htmlspecialchars($value).'" style="width:100%;">';
	} elseif($type == 'int') {
		$input = '<input type="text" name="'.$name.'" value="'.jd_htmlspecialchars($value).'" size="15">';
	} elseif($type == 'pass') {
...
function runinfo($message = '') {
	...
	if(isset($db)) {
		if(empty($ifdebug)) {
			...
			foreach($db->queries as $query) {
				$str_debug .= "<li>".jd_htmlspecialchars($query)."</li>\n";
			}
	...
	$str_debug = jd_replace("</body>", "$str_debug\n".getcopyrightinfo()."\n</body>", ob_get_contents());
	ob_end_clean();
	echo($str_debug);
}
...
function createfore() {
	...
	writetofile($config_data, FORE_ROOT.'jdcms_config.php');
	$files = array('attachment', 'captcha', 'category', 'comment', 'inc', 'include', 'item', 'keyword', 'page', 'post', 'rounter', 'score', 'section', 'user');
	foreach($files as $file) {
		$content = "<?php include 'jdcms_config.php';\$file = '{$file}';include \$system_root.'/fore.php';?>";
		writetofile($content, FORE_ROOT.'jdcms_'.$file.'.php');
	}
	jd_copy(CORE_ROOT.'include/jquery.js', FORE_ROOT.'jds/jquery.js');
	jd_copy(CORE_ROOT.'install/user.css', FORE_ROOT.'jds/user.css');
}
...
function extfieldinput($field) {
	$type = $field['type'];
	$standby = jd_htmlspecialchars($field['standby']);
...
function rendermodulefield($key, $data, $value = false) {
	...
	if(!empty($height) && $key != 'paging') {
		...
		if($data['type'] == 'plain') {
			...
			$htmlfields .= "<td><input type=\"text\" name=\"$key\" value=\"".jd_htmlspecialchars($value)."\" style='width:{$width}'>";
		}
	} elseif($key == 'picture') {
		...
		$htmlfields .= "<div id='otherattach'><input class='keeporiginal' type='checkbox' name='keeporiginal' value='1'>{$lan['keeporiginal']}<br></div><input type='button' value='{$lan['add']}{$lan['space']}{$lan['attach']}' onclick='addattach()'><script>addattach();</script>";
	} elseif($key == 'paging') {
		...
		if(substr($width, -1) != '%') $width .= 'px';
		$value = jd_htmlspecialchars($value);
		...
		if(strpos($data['default'], ';') === false) {
			...
			if(!empty($value)) $htmlfields .= "<script>$('#{$key}').val(\"".jd_htmlspecialchars($value)."\");</script>";
		}
	}
...
function ifitemtemplateexist($category, $template = '') {
	...
	if(empty($template)) $template = $categorycache['itemtemplate'];
 	$templatefile = JD_ROOT."configs/templates/$template_path/,".$template;
...
<td><input type='text' name='{$field}_default' size='10' value='{$_default}'></td>";
	...
	foreach($data as $_k => $_v) {
		...
		$fieldshtml .= "<tr><td><input name='extfield{$trid}' type='text' size='12' value='".jd_htmlspecialchars($k)."'></td><td>\${$_k}</td><td>[{$_k}]</td><td><input type='text' name='extfield_alias{$trid}' size='10' value='".jd_htmlspecialchars($_alias)."'></td><td><input type='text' name='extfield_order{$trid}' size='3' value='".htmlspecialchars($_order)."'></td>
	<td><input type='text' name='extfield_listorder{$trid}' size='3' value='".jd_htmlspecialchars($_listorder)."'></td>
	<td><input type='text' name='extfield_description{$trid}' size='15' value='".jd_htmlspecialchars($_description)."'></td>
	<td><input type='text' name='extfield_default{$trid}' size='10' value='".jd_htmlspecialchars($_default)."'></td><td><input type='text' name='extfield_size{$trid}' size='9' value='{$_size}'></td><td>{$setting}</td></tr>";
		$trid ++;
	}
	return $fieldshtml;
}
...
function renderitemfield($moduleid, $itemvalue = array()) {
	...
	foreach($fields as $key => $value) {
		...
		if($key == 'category') {
			...
			if(isset($itemvalue['id'])) $modulefield = jd_replace('id=[itemid]', "id={$itemvalue['id']}", $modulefield);
...
function showprocess($title, $processurl, $targeturl = '', $timeout = 100, $steps = array()) {
	displaytemplate('admincp_process.htm', array('title' => $title, 'processurl' => $processurl, 'targeturl' => $targeturl, 'timeout' => $timeout, 'steps' => $steps));
	runinfo();
	aexit();
}
...
function getmenus($id = 'system') {
	...
	if($id == 'custom') $config = JD_ROOT.'configs/custom.menu.xml';
	...
	$groups = array();
	foreach($values as $v) {
		// <group id="favorite" text="lan.kuaijielianjie"> 
		if($v['tag'] == 'GROUP' AND $v['type'] == 'open') {
			$gid = $v['attributes']['ID'];// $gid = "favorite"
			$groups[$gid] = array(
				'title' => getxmltext($v['attributes']['TEXT']), //得到的对应的是lan.kuaijielianjie在menu.php对应的文字
				'menus' => array()
			);
		}
		//<menu url="[homepage]*">lan.wangzhanshouye</menu>
		if($v['tag'] == 'MENU') {
			$url = $v['attributes']['URL']; //[homepage]*
			$url = str_replace('[homepage]', $homepage, $url);//替换掉[homepage]
			$text = getxmltext($v['value']);// lan.wangzhanshouye对应menu.php里的文字
...
function rendermenulink($menu) {
	if($menu['url'] == 'index.php?file=admincp&action=custom#') {
		if(!file_exists(JD_ROOT.'configs/custom.menu.xml')) return '';
	}
...
/*
 * 转换custom.menu.xml里例如 lan.lanmuliebiao
 * configs/language/menu.php中定义：
 * lanmuliebiao	栏目列表
 * 则把 lan.lanmuliebiao 转换为 栏目列表
 */
...
function h($params){
	...
	return "http://www.jdcms.com/manual/{$manual}.htm?source=cp";
}
